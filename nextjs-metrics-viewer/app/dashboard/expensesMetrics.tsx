...
import { selectTotalExpenses, selectAvgExpenses, selectMinExpense, selectMaxSExpense } from "../store/slices/productsSlice"
...
export default function ExpensesMetrics() {
  ...
  const averageExpenses = useAppSelector(selectAvgExpenses)
  const minExpense = useAppSelector(selectMinExpense)
  const maxExpense = useAppSelector(selectMaxSExpense)
      ...
      <div
        ...
        <dl className="grid max-w-screen-xl grid-cols-2 gap-8 mx-auto text-gray-900 sm:grid-cols-3 xl:grid-cols-6 dark:text-white sm:p-8" style={dataStyle}>
          <div className="flex flex-col items-center justify-center">
            <dt className="mb-2 text-2xl font-extrabold">${totalExpenses}</dt>
            <dd className="text-gray-500 dark:text-gray-400">Total Expenses</dd>
          </div>
          <div className="flex flex-col items-center justify-center">
            <dt className="mb-2 text-2xl font-extrabold">${averageExpenses.toFixed(1)}</dt>
            <dd className="text-gray-500 dark:text-gray-400">Average Expenses</dd>
          </div>
          <div className="flex flex-col items-center justify-center">
            <dt className="mb-2 text-2xl font-extrabold">${minExpense}</dt>
            <dd className="text-gray-500 dark:text-gray-400">Maximum Expenses</dd>
          </div>
          <div className="flex flex-col items-center justify-center">
            <dt className="mb-2 text-2xl font-extrabold">${maxExpense}</dt>
